...
int vm_run(ir_t *pc, int argc, char *argv[]) { // 虛擬機 => pc: 程式計數器, sp: 堆疊暫存器, bp: 框架暫存器
  ir_t i, *sp; // *bp, a: 累積器, cycle: 執行指令數
  obj_t *a; //, *t;
  int cycle;    // temps
  ...
  */
  ...
  while (1) {
    ...
    /*if      (i == Lea) a = (ir_t)(bp + *pc++);                             // load local address 載入區域變數
    else if (i == Jsr) { // jump to subroutine 跳到副程式
        *--sp = (ir_t)(pc + 1); // 推返回點入堆疊
        pc = (ir_t *)*pc; // goto 副程式
    }
    else if (i == Narg) {
        ir_t n = *pc++;
    ...
    // else if (i == Jmp) pc = (ir_t *)*pc;                                   // jump               躍躍指令
    // else if (i == Bz)  pc = a ? pc + 1 : (ir_t *)*pc;                      // branch if zero     if (a==0) goto m[pc]
    // else if (i == Bnz) pc = a ? (ir_t *)*pc : pc + 1;                      // branch if not zero if (a!=0) goto m[pc]
    // else if (i == Ent) { *--sp = (ir_t)bp; bp = sp; sp = sp - *pc++; }     // enter subroutine   進入副程式
    // else if (i == Adj) sp = sp + *pc++;                                   // stack adjust       調整堆疊
    // else if (i == Lev) { sp = bp; bp = (ir_t *)*sp++; pc = (ir_t *)*sp++; } // leave subroutine   離開副程式
    // else if (i == Load) {
    //     int idx = (ir_t *)a;  
    //     a = *                                   // load int           載入整數
    // else if (i == Ldc)  a = *(char *)a;                                    // load char          載入字元
    // else if (i == Sti)  *(ir_t *)*sp++ = a;                                 // store int          儲存整數
    ...
    // else if (i == Malc) a = (ir_t)malloc(*sp); // 分配記憶體
    // else if (i == Free) free((void *)*sp); // 釋放記憶體
    // else if (i == Mset) a = (ir_t)memset((char *)sp[2], sp[1], *sp); // 設定記憶體
