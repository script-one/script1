...
int main() {
    ...
    ForEach<list<char>>(chars, [](char &chr){ cout << chr << endl; });
    ForEach<vector<int>>(nums, [](int &num) { cout << num << endl; });
    ForEach<string>(word,[](char &letter) { cout << letter << endl; });
    ...
    ForEach<vector<int>>(mappedNums, [](int &x) { cout << x << endl; });
